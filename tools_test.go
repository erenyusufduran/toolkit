...
var slugTests = []struct {
	...
	{name: "complex string", s: "Now is the time for all GOOD men! + fish & sucha s ^&2", expected: "now-is-the-time-for-all-good-men-fish-sucha-s-2", errorExpected: false},
	{name: "japanese string", s: "こんにちは世界", expected: "", errorExpected: true},
	{name: "japanese string and roman chars", s: "こんにちは世界 hello world", expected: "hello-world", errorExpected: false},
}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	var testTools Tools
	testTools.DownloadStaticFile(rr, req, "./testdata", "pic.jpeg", "puppy.jpeg")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "6063" {
		t.Error("wrong content length of", res.Header["Content-Length"])
	}
...
var jsonTests = []struct {
	...
	{name: "two json files", json: `{"foo": "bar"}{"er": "ko"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
